...
const HistoryBackground = () => {
    const histories = [
        ...
        {
            year: "2022",
            title: "Joined Mbeya University of Science and Technology",
            side: "right"
        }
    ]
    return (
        <div className="">
            <h3 className="text-4xl md:text-5xl text-center bg-gradient-to-r from-cyan-400 to-purple-500 text-transparent bg-clip-text">Historical Background</h3>
                ...
                <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-cyan-400 to-purple-500" />
                    ...
                    {histories.map((history, index) => (
                        <div key={index} className={`relative mb-16 flex ${history.side === "left" ? "justify-start" : "justify-end"}`}>
                            ...
                            {/* Branch dot */}
                            <div className="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 rounded-full border-4 border-gray-900"></div>
